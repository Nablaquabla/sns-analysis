...
def createCondorFile(dataDir,outDir,run,day,time):
    ...
    with open('/home/bjs66/CondorFiles/%s-%s-%s.condor'%(run,day,time),'w') as f:
        ...
        f.write('Executable = /home/bjs66/GitHub/sns-analysis/sns-muon-veto\n')
...
#    run = 'Run-16-03-22-18-09-33' 
    ...
    runDirs = ['Run-15-06-25-12-53-44','Run-15-08-18-14-51-18','Run-15-10-13-13-27-09','Run-15-11-09-11-30-13']
    subdirs = {}
    days_in = {}
    read_subdirs = False
    read_days = False
    ...
    for run in runDirs:           
        for day in days_in[run]:
            ...
            # Get all times within the day folder chosen
            outputList = [x.split('-')[1] for x in os.listdir(outDir) if 'Data-' in x]
                        
            # Check if there is a file missing in the day folder
            if len(inputList) != len(outputList):
                missing = list(set(inputList) - set(outputList))
		print missing
                if len(missing) > 0: 
                    print len(missing)
                if runMissing == '1':
